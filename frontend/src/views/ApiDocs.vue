<template>
  <div class="api-docs">
    ...
    <p class="intro">本系统提供标准的RESTful API接口，供第三方系统接入招聘数据。</p>
    ...
    <a-tabs v-model:activeKey="activeTab">
      ...
      <a-tab-pane key="auth" tab="认证接口">
        <a-card title="1. 用户登录" style="margin-bottom: 16px">
  ...
  timestamp: "2024-01-01T00:00:00"
      ...
      <!-- 招聘数据接口 -->
      <a-tab-pane key="jobs" tab="招聘数据接口">
        <a-card title="1. 获取招聘列表" style="margin-bottom: 16px">
          <a-descriptions bordered :column="1">
            <a-descriptions-item label="接口地址">
            ...
            <a-descriptions-item label="查询参数">
              <a-table
                :columns="jobsParamsColumns"
                :data-source="jobsParams"
                :pagination="false"
                size="small"
              />
            </a-descriptions-item>
  ...
  data: {
    ...
    pagination: {
      ...
      pages: 5
        ...
        <a-card title="2. 搜索招聘信息" style="margin-bottom: 16px">
          <a-descriptions bordered :column="1">
            <a-descriptions-item label="接口地址">
              <a-tag color="green">GET</a-tag>
      ...
      <!-- 监控规则接口 -->
      <a-tab-pane key="monitoring" tab="监控规则接口">
        <a-card title="1. 创建监控规则" style="margin-bottom: 16px">
          <a-descriptions bordered :column="1">
            <a-descriptions-item label="接口地址">
              <a-tag color="blue">POST</a-tag>
...
Content-Type: application/json</pre>
  ...
  is_active: true
        ...
        <a-card title="2. 获取监控规则列表" style="margin-bottom: 16px">
          <a-descriptions bordered :column="1">
            <a-descriptions-item label="接口地址">
              <a-tag color="green">GET</a-tag>
              <code>/api/v1/monitoring-rules</code>
      ...
      <!-- 扫描结果接口 -->
      <a-tab-pane key="results" tab="扫描结果接口">
        <a-card title="1. 获取扫描结果列表" style="margin-bottom: 16px">
          <a-descriptions bordered :column="1">
            <a-descriptions-item label="接口地址">
              <a-tag color="green">GET</a-tag>
...
const jobsParams = [
  ...
  { name: 'status', type: 'string', required: '否', desc: '状态过滤：active/inactive' },
  { name: 'city', type: 'string', required: '否', desc: '城市过滤' },
  { name: 'keyword', type: 'string', required: '否', desc: '关键词搜索' }
]

// 搜索参数
const searchParamsColumns = jobsParamsColumns
const searchParams = [
...
// 监控规则参数
const ruleParamsColumns = jobsParamsColumns
const ruleParams = [
...
// 扫描结果参数
const resultParamsColumns = jobsParamsColumns
const resultParams = [
  { name: 'page', type: 'integer', required: '否', desc: '页码，默认1' },
  { name: 'per_page', type: 'integer', required: '否', desc: '每页数量，默认20' },
